<?php
namespace Kayako\Api\Client\Common\REST;
...
class RESTClient implements RESTClientInterface {
	 ...
	 * @see RESTClientInterface::setConfig()
	 *
	 * @param Config $config
	 */
	...
	protected function processRequest($controller, $method, $parameters = array(), $data = array(), $files = array()) {
		...
		if ($response === false)
			throw new GeneralException(sprintf('CURL error: %s (%s)', curl_error($curl_handle), curl_errno($curl_handle)));

		$http_status = curl_getinfo($curl_handle, CURLINFO_HTTP_CODE);
		if ($http_status != 200)
			throw new GeneralException(sprintf("HTTP error: %s", $http_status));
		...
		$result = Helper::convertXmlToArray($response);
		if ($result === false)
			throw new GeneralException("Error parsing XML response.");
	 ...
	 *
	 * {@inheritdoc}
	 * @see RESTClientInterface::delete()
	 */
