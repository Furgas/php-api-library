...
class ResultSet implements \Iterator, \Countable, \ArrayAccess {
	 ...
	 *
	 * @param ObjectBase[]|ResultSet $objects List of objects in this result set.
	 * @param string $class_name Optional. Class name of objects in this result set.
	 * @param ResultSet $previous_result_set Optional result set which was the base of filtering operation producing this result set.
	 */
	...
	public function offsetSet($offset, $value) {
		if (!is_object($value) || (strlen($this->class_name) > 0 && get_class($value) !== $this->class_name))
			throw new GeneralException(sprintf('The result set can only hold objects of type "%s"', $this->class_name));
	 ...
	 *
	 * NOTICE: This method shouldn't be used directly in most cases. Use ResultSet->filterByXXX(filter_value1, filter_value2) instead. Call ObjectType::getAvailableFilterMethods() for possible filter methods.
	 *
	...
	public function filterBy($get_method_name, $filter_values) {
		...
		if (func_num_args() > 2) {
			$args = func_get_args();
			$get_method_arguments = array_splice($args, 2);
		}
		...
		return new ResultSet($filtered_objects, $this->getObjectsClassName(), $this);
	}
	 ...
	 * NOTICE: This method shouldn't be used directly in most cases. Use ResultSet->orderByXXX() instead. Call ObjectType::getAvailableOrderMethods() for possible order methods.
	...
	public function orderBy($get_method_name, $asc = true) {
		usort($this->objects, function($object1, $object2) use($get_method_name, $asc) {
			return ResultSet::compareObjects($object1, $object2, $get_method_name, $asc);
		});
		$this->object_keys = array_keys($this->objects);
		return $this;
	}
	 ...
	 *
	 * NOTICE: This method shouldn't be used directly in most cases. Use ResultSet->collectXXX() instead.
	 *
	 * Additional method arguments will be passed as arguments to the get method.
	 *
	...
	public function collect($get_method_name) {
		...
		if (func_num_args() > 1) {
			$args = func_get_args();
			$get_method_arguments = array_splice($args, 1);
		}
	...
	public function getPage($page_number, $items_per_page = 20) {
		...
		return new ResultSet($page_objects, $this->getObjectsClassName());
	}
	 ...
	 * To get available filter methods, call ObjectType::getAvailableFilterMethods().
	 * To get available order methods, call ObjectType::getAvailableOrderMethods().
	 *
	...
	public function __call($name, $arguments) {
		if (stripos($name, self::FILTER_PREFIX) === 0) {
			if (count($this->object_keys) === 0)
				return new ResultSet($this->objects, $this->getObjectsClassName());
	 ...
	 *
	 * @param ObjectBase $object1 First object to compare.
	 * @param ObjectBase $object2 Second object to compare.
