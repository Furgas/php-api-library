 ...
 * @see ObjectBase::$controller
 * @see ObjectBase::$object_xml_name
 *
...
abstract class ObjectBase {
	...
	static protected function getRESTClient() {
		return Config::get()->getRESTClient();
	}
	 ...
	 * @return ResultSet|static[]
	 */
	static protected function genericGetAll($search_parameters = array()) {
	 ...
	 *
	 * @param array $complete_id Object identifier or list of identifiers (ex. ticket identifier and ticket post identifier when fetching TicketPost).
	 * @return static
	 */
	static protected function genericGet($complete_id) {
		if (!is_array($complete_id)) {
			$complete_id = array($complete_id);
		}

		$result = self::getRESTClient()->get(static::$controller, $complete_id);
	...
	public function create() {
		...
		if (count($result) === 0)
			throw new GeneralException("No data returned by the server after creating the object.");
	...
	public function update() {
		...
		if (count($result) === 0)
			throw new GeneralException("No data returned by the server after updating the object.");
	...
	static private function initAPIFieldsAccessors() {
		...
		if (self::$_api_fields === null || !array_key_exists($classname, self::$_api_fields)) {
			...
			foreach ($class->getProperties(\ReflectionProperty::IS_PROTECTED | \ReflectionProperty::IS_PUBLIC) as $property) {
				...
				$parameters = Helper::getTagParameters($comment, 'apiField');
				if ($parameters === false)
					continue;
	 ...
	 * @throws GeneralException When there are missing field values and $throw_exception is true.
	...
	public function checkRequiredAPIFields($create, $throw_exception = true) {
		...
		if (count($missing_fields) > 0) {
			if ($throw_exception)
				throw new GeneralException(sprintf("Values for API fields '%s' is required for this operation to complete.", implode(', ', $missing_fields)));
	...
	static public function getAvailableFilterMethods($filter_names_only = true) {
		...
		if (!array_key_exists($class_name, self::$_filter_methods)) {
			...
			foreach ($class->getMethods(\ReflectionMethod::IS_PUBLIC) as $method) {
				...
				$parameters = Helper::getTagParameters($method_comment, 'filterBy');
				if ($parameters === false)
					continue;
				...
				$filter_methods[ResultSet::FILTER_PREFIX . $filter_method_name] = $get_method_name;
			}
	...
	static public function getAvailableOrderMethods($order_names_only = true) {
		...
		if (!array_key_exists($class_name, self::$_order_methods)) {
			...
			foreach ($class->getMethods(\ReflectionMethod::IS_PUBLIC) as $method) {
				...
				$get_method_name = $method->getName();
				$method_comment = $method->getDocComment();
				$parameters = Helper::getTagParameters($method_comment, 'orderBy');
				if ($parameters === false)
					continue;
				...
				$order_methods[ResultSet::ORDER_PREFIX . $order_method_name] = $get_method_name;
			}
	...
	public function __toString() {
		...
		$object_name = substr($class_name, strrpos($class_name, '\\')+1);

		return sprintf("%s (id: %s): %s\n", $object_name, implode(', ', $this->getId(true)), $this->toString());
	}
}